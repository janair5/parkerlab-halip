function updateCustomDataFields(iTrial, H)
global BpodSystem
...
if iTrial>0
    ...
    if isfield(BpodSystem.Data.RawEvents.Trial{iTrial},'Events') %not sure why this is necessary: why can there be trials without events if CenterPortIn is always necessary to proceed?
        ...
        if any(strcmp('Cin_Reward',statesThisTrial))
            ...
            RewardReceivedCenter = max((RewardDuration/CenterValveTime) * BpodSystem.Data.Custom.RewardAmountCenter(iTrial),0);
        else
        ...
        if any(strcmp('LinError_Fb',statesThisTrial))
            ...
            RewardReceivedError = max((RewardDuration/ErrorValveTime) * BpodSystem.Data.Custom.RewardAmountError(iTrial),0);
        else
...
if iTrial>0
    ...
    if isfield(BpodSystem.Data.Custom,'firstblock')
        ...
        % -- Rewrite line above to avoid stats toolbox --
        BiasTable = TaskParameters.GUI.BiasTable.Signal(TaskParameters.GUI.BiasTable.BlockLength>0);
        BpodSystem.Data.Custom.BlockBias = BiasTable(ceil(rand*length(BiasTable)));
        % -- End rewrite --
    end
    ...
    % -- Rewrite line above to avoid stats toolbox --
    BpodSystem.Data.Custom.BlockNoise=TaskParameters.GUI.BiasTable.Noise(ceil(rand*length(TaskParameters.GUI.BiasTable.Noise)));
    % -- End rewrite --
