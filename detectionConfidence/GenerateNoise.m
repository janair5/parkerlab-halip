function [att] =GenerateNoise(StimulusSettings, H)
...
switch NoiseColor
    ...
    case 'WhiteGaussian' %gaussian noise from mean 0 std .25
        error('Error: The HiFi module cannot render WhiteGaussian noise. Please select WhiteUniform.');
        noise = .25 * randn(1, samplenum);%make white gausian noise and clip to [-1 1]
        noise(noise<-1)=-1;
        noise(noise>1)=1;
    case 'PinkGaussian'
        error('Error: The HiFi module cannot render PinkGaussian noise. Please select WhiteUniform.');
