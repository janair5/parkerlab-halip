function psych = psychometricFit(choice,nCues_RminusL,fitNotBinned,bins,fitBinned,fitInverse)

% psych = psychometricFit(choice,nCues_RminusL,fitNotBinned,bins,fitBinned,fitInverse)
% fits psychometric functions for % right choice
...
for ii = 1:length(psych.perfPsych_xaxis)
  psych.nR(ii)        = sum(choice(nCues_RminusL==psych.perfPsych_xaxis(ii)) == Choice.R);
...
for ii = 1:length(psych.perfPsych_bins)-1
  psych.nR_binned(ii)           = sum(choice(nCues_RminusL>=psych.perfPsych_bins(ii) & ...
    nCues_RminusL<psych.perfPsych_bins(ii+1)) == Choice.R);
...
if fitInverse
  psych.sigmoid        = @(O,A,lambda,x0,x) 1 - (O + A ./ (1 + exp(-(x-x0)/lambda)));
else
